...
object HumanReborn: ModInitializer {
    ...
    val HUMAN_ENTITY_TYPE: EntityType<HumanEntity> = Registry.register(Registries.ENTITY_TYPE, HUMAN_IDENTIFIER, EntityType.Builder.create({ _, world -> HumanEntity(world) }, SpawnGroup.MONSTER).dimensions(0.6F, 1.8F).eyeHeight(1.62F).vehicleAttachment(PlayerLikeEntity.VEHICLE_ATTACHMENT).maxTrackingRange(8).build(RegistryKey.of(RegistryKeys.ENTITY_TYPE, HUMAN_IDENTIFIER)))
    private val HUMAN_SPAWN_EGG = SpawnEggItem(Item.Settings().spawnEgg(HUMAN_ENTITY_TYPE).registryKey(RegistryKey.of(RegistryKeys.ITEM, SPAWN_EGG_IDENTIFIER)))
